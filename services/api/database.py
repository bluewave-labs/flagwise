...
class DatabaseService:
    ...
    def get_forecast(self, time_range="daily", date_range="7d", forecast_days=7, provider=None, model=None):
        """Generate predictive forecasts for request volume and threat trends"""
        try:
            # First get historical data using existing methods
            volume_data = self.get_volume_trends(time_range, date_range, provider, model)
            threat_data = self.get_threat_trends(time_range, date_range, provider, model)
            ...
            def simple_forecast(data_points, forecast_periods):
                if len(data_points) < 2:
                    return []
                
                # Extract y values and create x values (time indices)
                y_values = np.array([float(point.get('requests', 0)) for point in data_points])
                ...
                if len(y_values) > 1:
                    ...
                    m = np.sum((x_values - x_mean) * (y_values - y_mean)) / np.sum((x_values - x_mean) ** 2)
                    b = y_mean - m * x_mean
                    
                    # Generate forecast points
                    forecast_points = []
                    last_time = data_points[-1].get('time', '')
                    
                    for i in range(1, forecast_periods + 1):
                        forecast_x = len(y_values) + i - 1
                        forecast_y = max(0, int(m * forecast_x + b))  # Ensure non-negative
                        
                        # Generate future time labels
                        if time_range == 'daily':
                            future_date = datetime.now() + timedelta(days=i)
                            time_label = future_date.strftime('%m-%d')
                        elif time_range == 'hourly':
                            future_date = datetime.now() + timedelta(hours=i)
                            time_label = future_date.strftime('%H:%M')
                        else:
                            time_label = f"+{i}"
            ...
            def threat_forecast(data_points, forecast_periods):
                if len(data_points) < 2:
                    return []
                
                # Extract threat rate values
                threat_values = np.array([float(point.get('threatRate', 0)) for point in data_points])
                x_values = np.array(range(len(threat_values)))
                
                if len(threat_values) > 1:
                    # Calculate trend
                    x_mean = np.mean(x_values)
                    y_mean = np.mean(threat_values)
                    
                    m = np.sum((x_values - x_mean) * (threat_values - y_mean)) / np.sum((x_values - x_mean) ** 2)
                    b = y_mean - m * x_mean
                    ...
                    for i in range(1, forecast_periods + 1):
                        forecast_x = len(threat_values) + i - 1
                        forecast_y = max(0, min(100, m * forecast_x + b))  # Clamp between 0-100%
            ...
            # Generate forecasts
            volume_forecast = simple_forecast(volume_data, forecast_days)
            threat_forecast_data = threat_forecast(threat_data, forecast_days)
            
            # Calculate confidence metrics based on data consistency
            def calculate_confidence(data_points):
                if len(data_points) < 3:
                    return 'low'
                
                values = [point.get('requests', 0) for point in data_points]
                if not values:
                    return 'low'
            ...
            volume_confidence = calculate_confidence(volume_data)
            threat_confidence = calculate_confidence(threat_data)
            
            return {
                'volume_forecast': volume_forecast,
                'threat_forecast': threat_forecast_data,
                'historical_data': {
                    'volume': volume_data[-5:] if len(volume_data) > 5 else volume_data,  # Last 5 points
                    'threat': threat_data[-5:] if len(threat_data) > 5 else threat_data
                },
                'confidence_metrics': {
                    'volume_accuracy': volume_confidence,
                    'threat_accuracy': threat_confidence,
